...
export const fetchAllPosts = async (): Promise<Post[]> => {
  return getCachedRequest('fetchAllPosts', async () => {
    try {
      ...
      console.log("[v0] Raw posts from DB:", JSON.stringify(posts, null, 2));
      // Filter out posts from private users and posts with images
      const filtered = posts
        .filter(post => !post.author?.is_private && !post.content.includes('<img'))
        .map(transformPostFromDB);
      console.log("[v0] Filtered posts (all) to display:", JSON.stringify(filtered, null, 2));
      return filtered;
    } catch (error) {
...
export const fetchUserPosts = async (username: string): Promise<Post[]> => {
  return getCachedRequest(`fetchUserPosts_${username}`, async () => {
    try {
      ...
      return posts ? posts.map(transformPostFromDB) : [];
    } catch (error) {
...
export const fetchUserUnpinnedPostsPaginated = async (username: string, page: number, limit: number): Promise<{ posts: Post[], total: number }> => {
  try {
    ...
    return { posts: posts ? posts.map(transformPostFromDB) : [], total: count || 0 };
  } catch (error) {
...
const transformPostFromDB = (dbPost: any): Post => {
  ...
  return {
    ...
    author: dbPost.author?.username || 'Unknown',
