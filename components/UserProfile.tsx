...
// Client-side cache for user profile and posts to make loading instantaneous
const userProfileCache = new Map<string, { user: User; posts: Post[]; timestamp: number }>();
...
const UserProfile: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (cached) {
                setUser(cached.user);
                setIsPrivate(cached.user.is_private || false);
                setPosts(cached.posts);
                const pinned = cached.posts.filter(post => post.pinned);
                const unpinned = cached.posts.filter(post => !post.pinned);
                setPinnedPosts(pinned);
                setUnpinnedPosts(unpinned);
                setTotalUnpinnedPosts(unpinned.length);
                setTotalPosts(cached.posts.length);
                setIsLoading(false);
            } else {
                setIsLoading(true);
            }
